classdef (Sealed) globals < dynamicprops
    ...
    %   text file eegpipe.globals.txt, located in the same folder as this class
    %   definition file.
    %
    % * In a shared computing environment, it is not recommended to edit
    %   the eegpipe.globals.txt file, as this change will become inmmediately
    ...
    %   folder a eegpipe.globals.txt file. For instance, if a user would like to
    %   change the value of the FileNaming and Precision variables of
    %   package pset.import, he or she should perform these steps:
    %
    %   1) Create a file +globals/+pset/eegpipe.globals.txt. The folder
    %      +globals can be anywhere, as long as it is in MATLAB's search
    %      path.
    %
    %   2) Edit the contents of +globals/+pset/eegpipe.globals.txt so that
    %      it reads:
    %
    %      FileNaming   Random
    %      Precision    Double
    ...
    %   the values specified in the eegpipe.globals.txt file located in the same
    %   folder as this class definition file. An important note is that the
    %   user MAY NOT define variables in the user-defined eegpipe.globals.txt that
    %   do not exist already in the system eegpipe.globals.txt.
    %
    %
    %
    % See also: pset
    
    % Documentation: pset_eegpipe.globals.txt
    ...
    methods (Access = protected)
        function obj = globals
            ...
            obj = eegpipe.globals.read_file(obj);
